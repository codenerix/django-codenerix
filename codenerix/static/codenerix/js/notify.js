...
angular
    .module('codenerixNotify', ['ngResource'])
    ...
    .controller('AlarmsCtrl', [
        ...
        function(
            ...
            (function tick() {
                //            if (!withfocus) {
                //                    // Set a new timeout in 1 seconds because
                //                    we don't have focus and we will not do
                //                    anything
                ...
                // Notifications
                $scope.data = Notifications.query(
                    {},
                    function() {
                        // Everything went ok
                        $scope.connectionlost = 0;
                        // Update our public structure here to avoid flicking
                        // the screen values
                        $scope.pubdata = $scope.data;
                        // Show alerts that are waiting to be shown and we
                        // haven't shown yet
                        if ($scope.pubdata.head != undefined &&
                            $scope.pubdata.head.order != undefined) {
                            angular.forEach(
                                $scope.pubdata.head.order, function(alarm_id) {
                                    var found = false;
                                    var alarm = $scope.pubdata.body[alarm_id];
                                    angular.forEach(
                                        $scope.onscreen, function(screen_id) {
                                    ...
                                    if (found) {
                                        ...
                                        if ((alarm == null) || (!alarm.popup)) {
                                            ...
                                            inotify(
                                                notify_msg(
                                                    alarm, alertdate, alarm_id),
                                                alarm,
                                                alarm_id,
                                                true);

                                            // If no style was selected, apply
                                            // general one
                                            if (alarm.style == 'custom') {
                                                // Calculate font color
                                                var yiqcolor = getContrastYIQ(
                                                    alarm.color.substr(1))
                                                // Set color
                                                $('#notify' + alarm_id)
                                                    .parent()
                                                    .parent()
