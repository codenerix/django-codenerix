...
def flatatt(attrs):
    ...
    without a value. Otherwise, the value is formatted through its own dict of
    `attrs`, which can be useful to parametrize Angular directives.
...
class CheckboxFieldRendererMixin(object):
    def __init__(self, name, value, attrs, choices):
        ...
        if attrs.pop("multiple_checkbox_required", False):
            field_names = [
                format_html("{0}.{1}", name, choice)
                for choice, dummy in choices
            ]
        ...
        super(CheckboxFieldRendererMixin, self).__init__(
            name,
            value,
            attrs,
            choices,
...
class RadioFieldRendererMixin(object):
    def __init__(self, name, value, attrs, choices):
        ...
        super(RadioFieldRendererMixin, self).__init__(
            name,
            value,
            attrs,
            choices,
