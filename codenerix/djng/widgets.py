...
def flatatt(attrs):
    ...
    without a value. Otherwise, the value is formatted through its own dict
    of `attrs`, which can be useful to parametrize Angular directives.
...
#    def render(self):
#        """
#        Outputs a <ul ng-form="name"> for this set of choice fields to
#        nest an ngForm.
#        """
#        start_tag = format_html('<ul {0}>',
#           mark_safe(' '.join(self.field_attrs)))
#        output = [start_tag]
#        for widget in self:
...
#        tag_attrs = dict(attrs, type=self.input_type, name=name,
#           value=self.choice_value)
#        if 'id' in attrs:
#            tag_attrs['id'] = '{0}_{1}'.format(attrs['id'], self.index)
#        if 'ng-model' in attrs:
#            tag_attrs['ng-model'] = "{0}['{1}']".format(attrs['ng-model'],
#               self.choice_value)
#        if self.is_checked():
...
class CheckboxFieldRendererMixin:
...
#    """
#    Form fields of type 'MultipleChoiceField' using the widget
#    'CheckboxSelectMultiple' must behave slightly different from the original.
#     This widget overrides the default functionality.
#    """
#    renderer = CheckboxFieldRenderer
#
#    def implode_multi_values(self, name, data):
#        """
#        Due to the way Angular organizes it model, when Form data is
#        sent via a POST request, then for this kind of widget, the posted
#        data must to be converted into a format suitable
...
#        """
#        Due to the way Angular organizes it model, when this Form data is
#        sent using Ajax, then for this kind of widget, the sent data has
#        to be converted into a format suitable
#        for Django's Form validation.
#        """
...
class RadioFieldRendererMixin:
    def __init__(self, name, value, attrs, choices):
        attrs.pop("djng-error", None)
        self.field_attrs = []
        if attrs.pop("radio_select_required", False):
            self.field_attrs.append(
                format_html('validate-multiple-fields="{0}"', name),
            )
        super().__init__(
            name,
            value,
...
#    """
#    Form fields of type 'ChoiceField' using the widget 'RadioSelect' must
#    behave slightly different from the original. This widget overrides the
