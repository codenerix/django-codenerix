    ...
    def get_foreign(self, vurl, vform, vmodel, search, function):
        ...
        for field in self.autofill:
            ...
            if comma:
                ...
                comma = True
            html += "'{0}':{1}".format(field, field_filter.replace("<FORMNAME>", vform))
...
class DynamicSelect(DynamicSelectInputWidget, forms.widgets.Select):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        html += u"     style=\"{0}\"".format(vstyle)
        html += u"     repeat=\"value.id as value in options.{0}\"".format(vmodel)
        html += u"     refresh=\"{}\"".format(self.get_foreign(vurl, vform, vmodel, "$select.search", 'getForeignKeys'))
...
class DynamicInput(DynamicSelectInputWidget, forms.widgets.Input):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        # Render
        ticket = u"uib-typeahead=\"item.label for item in {}\"".format(self.get_foreign(vurl, vform, vmodel, "$viewValue", 'getAutoComplete'))
        ticket += u' typeahead-on-select="autoFillFields($item, $model, $label, $event)"'
        ticket += u' typeahead-wait-ms="800"'
        ticket += u' ng-change="resetAutoComplete()"'
