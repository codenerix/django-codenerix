...
class Command(BaseCommand, Debugger):
    ...
    def handle(self, *args, **options):
        ...
        if mode not in ["wwwdata", "apache", "suexec"]:
            self.print_help("", "")
            raise CommandError(
                "Allowed modes are suexec, apache or wwwdata ('apache' and 'wwwdata' option will force permissions to apache or www-data user, suexec won't touch permissions)",
            )
        ...
        if not options["noguess"]:
            cmd = "find {} -name 'locale' -exec ls -lR {{}} \\; | grep www-data".format(
                appdir,
            )
            ...
            if guess != mode:
                self.print_help("", "")
                raise CommandError(
                    "You have selected mode '{}' but I believe you are using '{}', if sure use --noguess".format(
                        mode,
                        guess,
                    ),
                )
        ...
        if ERROR:
            raise CommandError(
                "Some error has happened, can not keep going! (avoid using --noauto to let CODENERIX find a solution)",
            )
        ...
        if mode == "apache" or mode == "wwwdata":
            ...
            if status:
                ...
                if status:
                    ...
                    # No permissions
                    raise CommandError(
                        "You requested 'apache' or 'wwwdata' execution mode but you are not www-data and we can not become root",
                    )
        ...
        if options["clean"]:
            ...
            if key == "y":
                ...
                raise CommandError(
                    "You requested to clean all 'locale' folders but you answered NO to the previous question, can not keep going!",
                )
        ...
        if not options["compile"]:
            for (code, name) in settings.LANGUAGES:
                ...
                cmd = "{}{}/manage.py makemessages -v0 --symlinks --ignore env -l {}".format(
                    sudo,
                    basedir,
                    code,
                )  # --exclude is not working in some computers
                status, output = getstatusoutput(cmd)
                if status:
                    raise CommandError(output)

                cmd = "{}{}/manage.py makemessages -v0 --symlinks --ignore env -d djangojs -l {}".format(
                    sudo,
                    basedir,
                    code,
                )  # --exclude is not working in some computers
