...
class Command(BaseCommand, Debugger):
    ...
    imports = [
        ...
        (
            "django-multi-email-field==0.5",
            "from multi_email_field.forms import MultiEmailField",
            "pip install git+https://github.com/fle/django-multi-email-field.git",
        ),
        ...
        (
            "xhtml2pdf",
            "import xhtml2pdf",
            "git clone https://github.com/xhtml2pdf/xhtml2pdf - and, python2 setup.py install or python3 setup.py install - sudo apt-get remove python-setuptools - wget https://bootstrap.pypa.io/get-pip.py - sudo -H python get-pip.py - sudo -H pip install -U pip setuptools",
        ),
    ]
    ...
    def check_dependencies(self):
        self.debug(
            "Checking all libraries required for CODENERIX to work:",
            color="cyan",
        )
        ...
        for entry in self.imports:
            ...
            error, output = getstatusoutput(
                "{} -c '{}' 2>&1".format("python3", command),
            )
            if not error:
                # Library test passed, check version
                package = name.split("=")[0]
                error, output = getstatusoutput(
                    "pip freeze | grep -e '^{}=' || echo 'EMPTY'".format(
                        package,
                    ),
                )
                if not error:
                    ...
                    self.debug(
                        "ERROR while looking for the package in PIP for '{}'".format(
                            name,
                        ),
                        color="red",
                    )
                    self.debug(
                        " -> Test: pip freeze | grep -e '^{}=' || echo 'EMPTY'".format(
                            package,
                        ),
                        color="yellow",
                    )
                    self.debug(
                        "pip freeze | grep '{}' || echo 'EMPTY'".format(name),
                    )
                    self.error(error)
                ...
                self.debug(
                    "MISSING or wrong version for '{}'".format(name),
                    color="red",
                )
                self.debug(" -> Test: {}".format(command), color="yellow")
                self.debug("{} -c '{}'".format("python3", command))
                self.error(error)
                self.debug("OUTPUT: {}".format(output))
                self.debug("")
